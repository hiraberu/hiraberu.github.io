...
function ApplyTotalValue(){
  var totalValue = document.getElementById( "TotalValue" ).value;
  console.log("TotalValue: " + totalValue);
  const splitData = totalValue.split(",");
  if( splitData.length < 22 ){ return; }
  document.getElementById( "input_title" ).value = splitData[0];
  document.getElementById( "Title" ).textContent = splitData[0];
  document.getElementById( "input_nickname" ).value = splitData[1];
  document.getElementById( "NickName" ).textContent = splitData[1];
  ChangeGender( splitData[2] );
  ChangePartsNo( "uc", splitData[3] );
  ChangePartsColor( "uc", splitData[4] );
  ChangePartsNo( "lc", splitData[5] );
  ChangePartsColor( "lc", splitData[6] );
  ChangePartsNo( "sh", splitData[7] );
  ChangePartsColor( "sh", splitData[8]);
  ChangePartsNo( "ha", splitData[9] );
  ChangePartsColor( "ha", splitData[10] );
  ChangePartsNo( "ba", splitData[11] );
  ChangePartsColor( "ba", splitData[12] );
  ChangePartsNo( "hair", splitData[13] );
  ChangePartsColor( "hair", splitData[14] );
  ChangePartsNo( "face", splitData[15] );
  ChangePartsNo( "ff", splitData[16] );
  ChangePartsNo( "eb", splitData[17] );
  ChangePartsNo( "eye", splitData[18] );
  ChangePartsColor( "eye", splitData[19] );
  ChangePartsNo( "mouth", splitData[20] );
  ChangePartsColor( "skin", splitData[21] );
...
function ChangePartsNo( key, param ){
  ...
  for( let i = 0; i < BtnNoList[key].length; i++ ){
    BtnNoList[key][i].style.border = parseInt( BtnNoList[key][i].getAttribute("id").slice(key.length+5) ) == PartsNo[key] ? "2px solid " + btn_select_color : "";
  }
  MakeTotalValue();
...
function ChangePartsColor( key, param ) {
  ...
  for( let i = 0; i < BtnColList[key].length; i++ ){
    ...
    BtnColList[key][i].style.border = ( parseInt( cno ) == PartsCol[key] ? "3" : "2" ) + "px solid "+color_table[cno][1];
    BtnColList[key][i].style.color = parseInt( cno ) == PartsCol[key] ? ((key == "skin") ? "#ff0000" : "#ffffff" ) : "#000000";
  }
  MakeTotalValue();
  if( PartsFile.includes(key) ){
    ChangeSvgFile(key); 
...
function ChangeGender( param ){
  gender = param;
  for( let i = 0; i < BtnGender.length; i++ ){
    BtnGender[i].style.border = BtnGender[i].getAttribute("value") == gender ? "2px solid "+ btn_select_color : "";
  }
  MakeTotalValue();
